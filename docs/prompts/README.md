...
Эта директория не является стартовым контекстом AI-агента. Обычные задачи должны начинаться с `AGENTS.md` и минимального набора source-of-truth документов по типу работы, а не с полного чтения `docs/prompts/**`.
...
## Как читать при явном использовании prompt library

1. Если пользователь просит применить prompt library целиком, начните с `context/project.md`.
2. Если пользователь просит конкретный role, workflow или task template, откройте только этот файл и `context/project.md` при необходимости.
3. Затем выберите role prompt из `agents/` под текущую задачу.
4. Если нужен общий порядок работы, используйте workflow prompt из `workflows/`.
5. Если задача типовая, используйте подходящий task template из `tasks/`.
