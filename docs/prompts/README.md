...
Для Codex прямого аналога `.qwenignore` в репозитории не используется. Codex автоматически подхватывает `AGENTS.md`, а не все Markdown-файлы как инструкции. Для более жесткой защиты можно настроить filesystem permissions в `.codex/config.toml`, запретив чтение `docs/prompts/`; пример есть в `.codex/config.example.toml`.
