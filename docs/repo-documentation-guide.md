...
| Базовая архитектура v1 и целевое устройство MVP | `docs/specs/01-avatar-service-v1.md` | Архитектура, модули, execution model, success criteria, non-MVP |
| Узкие change-specs | `docs/specs/*.md` кроме `01-avatar-service-v1.md` и `docs/specs/README.md` | Изолированные изменения вроде `/health`, их scope и acceptance criteria |
...
3. Если документ фиксирует целевое поведение или архитектурное решение, используйте `docs/specs/`.
4. Если документ описывает порядок реализации, rollout или sequence of work, используйте `docs/plans/`.
5. Не создавайте новый документ ради пересказа `README.md`, confirmed requirements или v1 spec.
...
- Изменился архитектурный baseline MVP: обновляйте `docs/specs/01-avatar-service-v1.md`.
...
| --- | --- | --- |
| API, statuses, delete/fallback, error model | endpoints, required headers, status codes, supported params, visibility rules | `docs/requirements/confirmed-requirements.md`, `docs/specs/01-avatar-service-v1.md`, `internal/http`, `internal/service`, `tests/contract/` |
| Web upload/gallery | routes, form field `file`, gallery filtering, отсутствие `POST /web/upload` | `docs/requirements/confirmed-requirements.md`, `docs/specs/01-avatar-service-v1.md`, `web/static/index.html`, `internal/http` |
...
- Если создается новый долгоживущий документ в `docs/requirements/`, `docs/specs/`, `docs/plans/` или корне `docs/`, его нужно сделать обнаруживаемым минимум из одного индексного документа.
...
- Для `docs/specs/` и `docs/plans/` поддерживайте directory-level `README.md` с краткой картой документов.
- Если новый документ временный, change-specific или узкий по scope, достаточно ссылки из directory `README.md` и из документа-владельца темы.
